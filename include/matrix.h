...
public:
  ...
  operator mat<NewRows, NewCols>() const {
    mat<NewRows, NewCols> result(1.0f); // Initialise avec des 0 et 1 sur la diagonale
    
    // Remplissage en haut à gauche
    ...
    // Remplissage de la diagonale avec 1.0f pour les nouvelles lignes et colonnes
  ...
  invert() {
    mat augmented = *this;
    mat result; // Suppose que `result` est initialisé à l'identité
    
    for (size_t i = 0; i < Rows; ++i) {
      float pivot = augmented(i, i);
      
      // Trouver un pivot non nul si besoin
      if (pivot == 0) {
        bool found = false;
      ...
      // Réduction des lignes en dessous
    ...
    // Réduction des lignes au-dessus pour obtenir la forme échelonnée réduite
  ...
  rotate(const vec3& vector) {
    ...
    // Calcul des cosinus et sinus pour chaque angle
    ...
    // Multiplier la matrice actuelle par la matrice de rotation combinée
